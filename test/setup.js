...
global.CSS = {
  supports: jest.fn().mockImplementation((prop, value) => {
    ...
    if (typeof prop === 'string') {
      ...
      return supportedFeatures.some(feature => prop.includes(feature.split(':')[0]));
    }
...
Object.defineProperty(window, 'matchMedia', {
  ...
  value: jest.fn().mockImplementation(query => ({
    matches: query.includes('width >= 768px') || query.includes('min-width: 1200px') || query === 'prefers-color-scheme: dark',
    media: query,
    onchange: null,
    addListener: jest.fn(),
